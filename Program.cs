...
class Program : ApplicationContext
{
    ...
    public Program()
    {
        Console.WriteLine("MADY BY F4DD (https://github.com/F4DD-dev)\n");
        LoadOrCreateConfig();
        ...
        {
            Icon = LoadIconFromResource("YandexMusicVolumeControl.app.ico"), // <-- здесь
    ...
    void Reconfigure()
    {
        Console.WriteLine("Configuring hotkeys. Enter key names (like F6, OemMinus, etc.):");
        Console.Write("Increase volume key: ");
        var ik = Enum.Parse<Keys>(Console.ReadLine(), true);
        Console.Write("Decrease volume key: ");
        var dk = Enum.Parse<Keys>(Console.ReadLine(), true);
        Console.Write("Mute/unmute key: ");
        var mk = Enum.Parse<Keys>(Console.ReadLine(), true);
        cfg = new Config { IncreaseKey = ik, DecreaseKey = dk, MuteKey = mk };
        File.WriteAllText(configPath, JsonSerializer.Serialize(cfg));
        Console.WriteLine("Configuration saved. Restart program.");
    }
